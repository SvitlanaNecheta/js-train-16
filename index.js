...
function filterStudentsByGrade(students, grade) {
  console.error("якщо ви бачите це повідомлення завдання 1 виконано не правильно");//Виведеме в консоль критичну помилку з текстом якщо ви бачите це повідомлення завдання 1 виконано не правильно
  console.clear();// Очищення консолі перед виведенням
  console.log("Завдання: 1 ==============================");// Виведемо повідомлення для відстеження роботи програми з текстом "Завдання: 1 =============================="
 for(const el of students){// Відфільтруємо тільки тих студентів оцінка яких співпадає з grade
  if(el.grade===grade){console.log(el)}// За допомогою перебору масиву виведемо повідомлення для відстеження роботи програми з іменем кожного студента,
...
function logArrayElements(arr) {
  ...
  console.log(i,":",arr[i])}}// Виводимо в консоль поточний елемент масиву та його індекс в форматі Елемент <індекс>: <значення>
}
...
function warnAboutMaxArrayLength(arr, maxLength) {
  if(!Array.isArray(arr)||isNaN(maxLength)){
    console.error("Перший аргумент має бути масивом, другий аргумент має бути числом!"); return null}
    else{
      if(arr.length>maxLength){console.warn("Увага! Довжина масиву перевищує максимально допустиму!")}// Перевіряємо, чи arr є масивом, а maxLength є числом.
...
function compareArrays(arr1, arr2) {
  ...
  if(res){console.log("масиви рівні")}// Сортуємо масиви за зростанням.
  else{console.assert(res,"Mасиви не однакові!")}// Оскільки напряму порівняти масиви за допомогою оператора === не можно, перетворимо ії в json і порівняємо
...
function buildMatrix(size) {
  if (isNaN(size)){console.error("Аргумент має бути числом!"); return null; }// Перевіряємо, чи size є числом.
  else{// Якщо size не є числом, виведеме в консоль критичну помилку з текстом "Аргумент має бути числом!".
  let arr=Array();// Повертаємо null, що свідчить про неможливість обробки вхідних даних.
  for(let i=0;i<size;i++){
    arr[i]=new Array()
    for(let j=0;j<size;j++){arr[i][j]=Math.trunc(Math.random()*10)}
...
function traceBackward(n) {
 console.log(n); // Виводимо поточне число n.
  if(n>1){traceBackward(n - 1)}// Якщо n є більше 1, викликаємо функцію traceBackward знову з аргументом n - 1.
  else if(n===1){console.trace()}// Виведемо стек викликів, коли досягнемо 1.
}
...
function displayGroupedInfo(students) {
 for(const el of students) {// Переберомо об'єкт за допомого for of
  console.group( `Студент: ${el.name}`);
  console.info(`Вік: ${el.age}`);
  console.info(`Предмет: ${el.subject}`);
  console.groupEnd();// Створимо групу з повідомленням `Студент: ${student.name}`
...
function validateUserInput(userInput) {
  ...
  console.assert(userInput.hasOwnProperty('username'),"Помилка: ім'я користувача відсутнє!");// Перевіремо умову наявності імені користувача, якщо воно відсутнє виводимо "Помилка: ім'я користувача відсутнє!"
  console.assert(userInput.hasOwnProperty('password'),"Помилка: пароль відсутній!");// Перевіремо умову наявності паролю, якщо він відсутній виводимо "Помилка: пароль відсутній!"
  if((userInput.password).length<8){console.warn("Попередження: пароль має бути довшим за 8 символів!")}// Перевірка довжини паролю чи менше вона ніж 8,якщо ні виводимо повідомлення про потенційну проблему з текстом "Попередження: пароль має бути довшим за 8 символів!"
}
...
function calculateTotalPrice(products) {
  console.time('My timer');// Початок вимірювання часу
 let total=0; // Створення змінної total яка буде нашою загальную ціною, початкове значення нуль
 for(const el of products) // Перебираємо кожен об'єкт товару та додаємо ціну товару до загальної вартості
  {total+=el.price;}
  ...
  console.timeEnd('My timer');// Зупинка вимірювання та виведення часу
}
...
const str="aeiou";// Створюємо рядок де будуть всі голосні "aeiou"
 for(let l of word) // Перебираємо кожну літеру у слові за допомогою for of
  {l=l.toLowerCase();
  // Перетворюємо літеру на малу літеру для порівняння
  
  if(str.includes(l)){ console.log(l);
    console.count('Лічильник голосних:')}
     else{console.count('Лічильник приголосних+!:')}}// Перевіряємо, чи є літера в нашому рядку з голосними,якщо так виводимио лічильник голосних, інакше приголосних
    console.countReset('Лічильник голосних:');
    console.countReset('Лічильник приголосних+!:');// Скидаємо лічильники
}
